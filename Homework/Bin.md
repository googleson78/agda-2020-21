...
```
Конструкторите са почти огледални на самото `Bin`, с разликата че базовият случай, `endI`, се конструира само двоичното число `end I`.
...
Ако конвертираме от канонично двоично число в естествено и обратно, получаваме оригиналното двоично число.
```agda
from-to-id-Can : (b : Bin) -> Can b -> fromNat (toNat b) == b
```
